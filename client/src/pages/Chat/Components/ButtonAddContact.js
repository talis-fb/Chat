...
function AddContact(props){
    ...
    return(
        <div className="form">
            <div>
                <input type="search" placeholder="Add Contact: #" maxLength={4} onChange={(e) => setPin( e.target.value )}  />
                <button className="add-contact" onClick={()=> props.addContact(pin)} type="submit">{icon}</button>
            </div>
        </div>
    )
